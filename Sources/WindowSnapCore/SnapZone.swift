...
public enum SnapZone {
    ...
    public static let EDGE_THRESHOLD: CGFloat = 6
    /// 隅の帯の幅(pt)。端に到達したうえで、角からこの距離以内にいれば隅(1/4)に倒す。
    public static let CORNER_BAND: CGFloat = 120
    ...
    public static func zone(forCursorGlobal p: CGPoint, screenFrameGlobal f: CGRect) -> SnapZone? {
        ...
        // 下端に到達: 左寄り→bottomLeft / 右寄り→bottomRight / 中央→なし（Dock 干渉回避）。
        if atBottom {
